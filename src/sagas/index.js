...
function* loadUser(login, requiredFields) {
  const gotUser = yield select(getUser, login);
  if (!gotUser || requiredFields.some(key => !Object.prototype.hasOwnProperty.call(gotUser, key))) {
...
function* loadRepo(fullName, requiredFields) {
  const gotRepo = yield select(getRepo, fullName);
  if (!gotRepo || requiredFields.some(key => !Object.prototype.hasOwnProperty.call(gotRepo, key))) {
