...
describe('Cart component', () => {
  it('should display total', () => {
    const { p } = setup('76')
    expect(p.text()).toMatch(/^合計: 76円/)
  })

  it('should display add some products message', () => {
    const { em } = setup()
    expect(em.text()).toMatch(/^カートが空です。/)
  })
  ...
  describe('when given product', () => {
    ...
    it('should render products', () => {
      const { products } = setup('10', product)
      const props = {
    ...
    it('should not disable button', () => {
      const { button } = setup('10', product)
      expect(button.prop('disabled')).toEqual('')
    })

    it('should call action on button click', () => {
      const { button, actions } = setup('10', product)
